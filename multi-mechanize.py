...
def run_test(remote_starter=None):
    ...
    for i, ug_config in enumerate(user_group_configs):
        ug = UserGroup(queue, i, ug_config.name, ug_config.num_threads, ug_config.script_file, run_time, rampup, run_localtime)
...
class UserGroup(multiprocessing.Process):
    def __init__(self, queue, process_num, user_group_name, num_threads, script_file, run_time, rampup, run_localtime):
        ...
        self.run_localtime = run_localtime
    ...
    def run(self):
        ...
        for i in range(self.num_threads):
            ...
            if i > 0:
                time.sleep(spacing)
            agent_thread = Agent(self.queue, self.process_num, i, self.start_time, self.run_time, self.user_group_name, self.script_file, self.run_localtime)
...
class Agent(threading.Thread):
    def __init__(self, queue, process_num, thread_num, start_time, run_time, user_group_name, script_file, run_localtime):
    ...
    def run(self):
        ...
        if self.script_file.lower().endswith('.py'):
            ...
            return
        try:
            trans = eval(module_name + '.Transaction(self.run_localtime)')
        except NameError, e:
